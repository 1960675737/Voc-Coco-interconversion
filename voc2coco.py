            ...
            with open(json_file, 'r', encoding="utf-8") as fp:
                ...
                for p in fp:
                    ...
                    if 'filename' in p:
                        ...
                        self.filen_ame=folder+'_'+self.filen_ame
                    ...
                    if flag == 1:
                        self.supercategory = self.ob[0]                          # name
                        if self.supercategory not in self.label:
                            self.categories.append(self.categorie())
                            self.label.append(self.supercategory)                # name
                        ...
                        self.area=self.bbox[2]*self.bbox[3]     # 改
...
# test file transfer from .xml to .json

# xml_file = glob.glob('../XmlFile/test/*.xml')
# PascalVOC2coco(xml_file, './test.json')

# trainval file transfer from .xml to .json

xml_file = glob.glob('../XmlFile/trainval/*.xml')
PascalVOC2coco(xml_file, './trainval.json')
